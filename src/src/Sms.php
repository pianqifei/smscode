...
class Sms
{
    ...
    public function sendCodeAndSave($phone,$type=1)
    {
        ...
        if(Str::startsWith($phone,'+86')===true){
            $lang=1;//zh-CN
        }elseif(Str::startsWith($phone,'+81')===true){
            $lang=3;//ja
        }elseif(Str::startsWith($phone,'+82')===true){
            $lang=4;//ko
        }else{
            $lang=2;
        }
        $content = trans('smscode::sms.sms_temp',['code'=>$code,'minutes'=>config('sms.timeout')],$lang_map[$lang]);
